App.controller('Menu', ['$scope', '$location', function ($scope, $location) {
    $scope.items = [
		...
		{ group: 'MY LIST' },
	    { text: 'Currently Watching', url: '/watching', icon: 'fa-eye', auth: true, selected: function () { $location.path() == this.url } },
        { text: 'Completed', url: '/completed', icon: 'fa-check', auth: true, selected: function () { $location.path() == this.url } },
        { text: 'Plan to Watch', url: '/todo', icon: 'fa-calendar', auth: true, selected: function () { $location.path() == this.url } },
        { text: 'Dropped', url: '/dropped', icon: 'fa-trash', auth: true, selected: function () { $location.path() == this.url } }
    ];
}]);